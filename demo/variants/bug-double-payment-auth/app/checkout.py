...
The authorization is taken and then captured, one call to the provider each,
so the funds are held and settled before the order is written.
"""
...
def checkout(req: CheckoutRequest):
    with connect() as conn:
        ...
        capture = authorize(conn, req.cart_id, total)
        if not (payment.approved and capture.approved):
